...
def get_result_types_three_qubit_circuit(
    theta, phi, varphi, obs, obs_targets, shots
) -> Circuit:
    circuit = (
        Circuit()
        .rx(0, theta)
...
def result_types_tensor_x_y_testing(device: Device, run_kwargs: Dict[str, Any]):
    ...
    obs_targets = [0, 2]
    circuit = get_result_types_three_qubit_circuit(
        theta, phi, varphi, obs, obs_targets, shots
    )
    tasks = (circuit, circuit.to_ir(ir_type=IRType.OPENQASM))
    for task in tasks:
...
def result_types_noncommuting_testing(device: Device, run_kwargs: Dict[str, Any]):
    ...
    obs3_targets = [0, 1, 2]
    circuit = (
        get_result_types_three_qubit_circuit(
            theta, phi, varphi, obs1, obs1_targets, shots
        )
...
def batch_bell_pair_testing(device: AwsDevice, run_kwargs: Dict[str, Any]):
    ...
    tasks_list = (
        circuits,
        [circuit.to_ir(ir_type=IRType.OPENQASM) for circuit in circuits],
    )
        ...
        if (layer + 1) % 100 != 0:
            for qubit in range(len(qubits)):
                ...
                gate = np.random.choice(
                    [Gate.Rx(angle), Gate.Ry(angle), Gate.Rz(angle), Gate.H()],
                    1,
                    replace=True,
                )[0]

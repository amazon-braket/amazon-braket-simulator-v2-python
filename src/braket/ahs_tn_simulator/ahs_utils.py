...
def show_drive_and_shift(drive: DrivingField, shift: ShiftingField):
    ...
    for ax, data_name in zip(axes, drive_data.keys()):
        max_value = max(drive_data[data_name].values())
        if data_name == 'phase [rad]':
            ax.step(drive_data[data_name].times(), np.array(drive_data[data_name].values())/max_value, '.-', where='post')
        else:
            ax.plot(drive_data[data_name].times(), np.array(drive_data[data_name].values())/max_value, '.-')
        ax.set_ylabel(data_name + " / max")
        ax.grid(ls=':')
        
    shift_data = shift.magnitude.time_series
    pattern = shift.magnitude.pattern.series   
    axes[-1].plot(shift_data.times(), np.array(shift_data.values())/max(shift_data.values()), '.-', label="pattern: " + str(pattern))
    axes[-1].set_ylabel('shift / max_shift')
    axes[-1].set_xlabel('time [s]')
    axes[-1].legend()
    axes[-1].grid()
    plt.tight_layout()
